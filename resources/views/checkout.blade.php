...
<div class="alert alert-danger alert-dismissible fade show rounded-3 mb-3" role="alert">
    ...
    <ul class="mb-0 mt-2 ps-3">@foreach($errors->all() as $e)<li style="font-size:13px">{{ $e }}</li>@endforeach</ul>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="checkout-section">
                ...
                <div class="row g-3">
                    <div class="col-sm-6">
                        ...
                        <input type="text" name="customer_name" value="{{ old('customer_name') }}" class="form-control" required placeholder="John Doe">
                    </div>
                    <div class="col-sm-6">
                        <label class="form-label fw-600" style="font-size:13px">Email <span class="text-danger">*</span></label>
                        <input type="email" name="customer_email" value="{{ old('customer_email') }}" class="form-control" required placeholder="email@example.com">
                    </div>
                    <div class="col-sm-12">
                        <label class="form-label fw-600" style="font-size:13px">Phone number <span class="text-danger">*</span></label>
                        <input type="tel" name="customer_phone" value="{{ old('customer_phone') }}" class="form-control" required placeholder="0912 345 678" id="phone">
                    </div>
                </div>
            </div>
            ...
            <div class="checkout-section">
                ...
                <textarea name="delivery_address" class="form-control" rows="3" required placeholder="Street address, ward, district, city">{{ old('delivery_address') }}</textarea>
            </div>

            {{-- Payment --}}
            <div class="checkout-section">
                <div class="section-label"><i class="fas fa-wallet"></i>Payment Method</div>
                <label class="pay-option {{ old('payment_method')=='cod'||!old('payment_method')?'active':'' }}" onclick="selectPay(this)">
                ...
                <label class="pay-option {{ old('payment_method')=='bank_transfer'?'active':'' }}" onclick="selectPay(this)">
                ...
                <label class="pay-option {{ old('payment_method')=='e_wallet'?'active':'' }}" onclick="selectPay(this)">
            ...
            <div class="checkout-section">
                ...
                <textarea name="notes" class="form-control" rows="2" placeholder="e.g. Morning delivery, leave at door...">{{ old('notes') }}</textarea>
            </div>
        </div>
            ...
            <div class="summary-card">
                ...
                <div>
                    ...
                    <div class="summary-item">
                        <span style="flex:1;color:var(--text)">{{ Str::limit($item['product']->name,35) }}</span>
                        <span class="text-muted ms-2">x{{ $item['quantity'] }}</span>
                        <span class="ms-3 fw-600" style="color:var(--danger);min-width:90px;text-align:right">${{ number_format($item['subtotal'], 2) }}</span>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="d-flex justify-content-between mt-3 pt-2" style="border-top:1px solid var(--border);font-size:14px">
                    ...
                    <span class="fw-600" id="subtotal-display">${{ number_format($total, 2) }}</span>
                </div>
                ...
                <a href="{{ route('cart.index') }}" class="btn btn-outline-secondary w-100 rounded-pill mt-2 fw-600">
...
function applyCoupon(){
    ...
    fetch('{{ route("coupon.apply") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},body:JSON.stringify({code,total:baseTotal})})
