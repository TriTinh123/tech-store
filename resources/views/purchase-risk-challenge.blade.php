            ...
            <div class="risk-bar-wrap">
                ...
                <div class="risk-bar-track">
                    <div class="risk-bar-fill"
                         style="width:{{ $fraud['risk_numeric'] ?? 50 }}%;background:{{ $barColor }}">
                    </div>
                </div>
                <span class="risk-bar-label" style="color:{{ $barColor }}">
                    {{ $fraud['risk_numeric'] ?? 50 }}/100
                </span>
            </div>
        </div>

        {{-- Reasons --}}
        @if(!empty($fraud['reasons']))
        <div class="reasons">
            <h4>Reason for verification</h4>
            <ul>
                @foreach($fraud['reasons'] as $reason)
                    <li>{{ $reason }}</li>
                @endforeach
            </ul>
        </div>
