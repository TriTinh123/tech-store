...
<div class="success-hero">
    ...
    <span class="os-eta-badge"><i class="fas fa-shipping-fast"></i>Estimated delivery: <strong>{{ $order->created_at->addDays(3)->format('d/m/Y') }} – {{ $order->created_at->addDays(5)->format('d/m/Y') }}</strong></span>
</div>
...
<div class="row g-4 justify-content-center">
    <div class="col-lg-7">
        ...
        <div class="os-tracker">
          ...
          {{-- Horizontal bar --}}
          <div class="os-hbar">
            <div class="os-hbar-fill" style="width:{{ $hbarW }}%"></div>
            @foreach(['Place Order','Confirm','Shipping','Received'] as $hi => $hl)
              <div class="os-hbar-step {{ $sIdx > $hi ? 'done' : ($sIdx === $hi ? 'cur' : '') }}">
                <div class="os-hbar-dot">{{ $sIdx > $hi ? '✓' : ($hi+1) }}</div>
                <div class="os-hbar-lbl">{{ $hl }}</div>
              </div>
            @endforeach
          </div>
          {{-- Vertical timeline --}}
          <div class="os-vtl" style="margin-top:20px">
            @foreach($vtSteps as $vi => $vs)
            <div class="os-vt-step {{ $vs['done'] ? 'done' : '' }} {{ $vs['cur'] ? 'cur' : '' }}">
              <div class="os-vt-line">
                <div class="os-vt-dot" style="{{ $vs['done'] ? 'background:'.$vs['color'].';border-color:'.$vs['color'] : '' }}">
                  @if($vs['done'])
                    <i class="fas {{ $vs['cur'] ? $vs['icon'] : 'fa-check' }}"></i>
                  @else
                    <i class="fas {{ $vs['icon'] }}"></i>
                  @endif
                </div>
                @if($vi < count($vtSteps)-1)
                <div class="os-vt-conn {{ $vs['done'] && $vtSteps[$vi+1]['done'] ? 'filled' : '' }}"></div>
                @endif
              </div>
              <div class="os-vt-body">
                <div class="os-vt-label" style="{{ $vs['done'] ? 'color:var(--text)' : '' }}{{ $vs['cur'] ? ';color:'.$vs['color'] : '' }}">
                  {{ $vs['label'] }}
                  @if($vs['cur'])<span class="os-cur-tag" style="background:{{ $vs['color'] }}">Current</span>@endif
                </div>
                <div class="os-vt-sub">{!! $vs['sub'] !!}</div>
                <div class="os-vt-time"><i class="fas fa-clock"></i> {{ $vs['time'] }}</div>
              </div>
            </div>
            @endforeach
          </div>
        ...
        <div class="os-card">
            ...
            <div class="px-4 py-2">
                <div class="info-row">
                    ...
                    <span class="info-val" style="color:var(--blue);font-size:16px">#{{ $order->order_number }}</span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <span class="info-val">
                        <span style="background:#dbeafe;color:#1e40af;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700">{{ ucfirst($order->status) }}</span>
                    </span>
                </div>
                <div class="info-row">
                    <span class="info-label">Total</span>
                    <span class="info-val" style="color:var(--danger);font-size:18px;font-weight:800">${{ number_format($order->total_amount, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        {{-- Products --}}
        <div class="os-card">
            <div class="os-card-head"><i class="fas fa-box-open"></i>Ordered Items <span class="badge rounded-pill text-bg-light ms-1">{{ $order->items->count() }}</span></div>
            <div class="px-4 py-2">
                ...
                <div class="order-item">
                    ...
                    <div class="fw-700" style="color:var(--danger);font-size:14px">${{ number_format($item->subtotal, 2) }}</div>
                </div>
                @endforeach
        ...
        <div class="os-card">
            ...
            <div class="px-4 py-3">
                <div class="fw-700 mb-1">{{ $order->customer_name }}</div>
                <div class="text-muted" style="font-size:14px">{{ $order->delivery_address }}</div>
                <div class="mt-1" style="font-size:14px"><i class="fas fa-phone me-2 text-success"></i>{{ $order->customer_phone }}</div>
                <div style="font-size:14px"><i class="fas fa-envelope me-2 text-success"></i>{{ $order->customer_email }}</div>
            </div>
        </div>
        ...
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:center;gap:12px;">
          ...
          <div>
            ...
            <div style="font-size:12px;color:#047857">Check your inbox <strong>{{ $order->customer_email }}</strong> for order details</div>
          </div>
        </div>

        {{-- Actions --}}
        <a href="{{ route('orders.show', $order) }}" class="btn-primary-ts">
            <i class="fas fa-box me-2"></i>View order details
        </a>
        <a href="{{ route('orders.index') }}" class="btn-outline-ts">
            <i class="fas fa-list me-2"></i>All my orders
        </a>
        <a href="{{ route('home') }}" class="btn-outline-ts" style="border-color:var(--border);color:var(--text-m)">
