    ...
    <div class="col-lg-8">
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h6>Recent Orders</h6>
                <a href="{{ route('admin.orders') }}" class="btn btn-sm btn-outline-secondary" style="font-size:12px">View all</a>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="bs bs-{{ $st }}">{{ $order->status ?? 'Pending' }}</span>
                            </td>
    ...
    <div class="col-lg-4">
        <div class="card h-100">
            ...
            <div class="card-body p-0">
                ...
                <div class="d-flex align-items-center gap-3 px-3 py-2 {{ $i + 1 < count($topProducts) ? 'border-bottom' : '' }}" style="border-color:#f1f5f9!important">
    ...
    <div class="col-lg-4">
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <span style="font-size:12px;color:var(--muted)">{{ $avgRating ?? 0 }}★ average</span>
            </div>
            <div class="card-body">
                @php $maxRating = max(array_values($ratingDistFull ?? [1])); if($maxRating == 0) $maxRating = 1; @endphp
                @foreach($ratingDistFull as $star => $count)
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
                    <span style="font-size:12px;font-weight:600;color:var(--text);width:14px;text-align:right">{{ $star }}</span>
                    <i class="fas fa-star" style="color:#f59e0b;font-size:11px;flex-shrink:0"></i>
                    <div style="flex:1;height:8px;background:#f1f5f9;border-radius:4px;overflow:hidden">
                        <div style="height:100%;width:{{ $maxRating > 0 ? round($count/$maxRating*100) : 0 }}%;background:{{ $star >= 4 ? '#22c55e' : ($star == 3 ? '#f59e0b' : '#ef4444') }};border-radius:4px;transition:width .4s"></div>
                    </div>
                    <span style="font-size:11px;color:var(--muted);width:28px;text-align:right">{{ $count }}</span>
                </div>
                @endforeach
                <div style="margin-top:14px;padding-top:12px;border-top:1px solid #f1f5f9;display:flex;justify-content:space-between;align-items:center">
                    ...
                    <span style="font-size:14px;font-weight:700;color:var(--text)">{{ $totalReviews ?? 0 }} reviews</span>
                </div>
            </div>
        </div>
    </div>

    {{-- Reviews by Day Line Chart --}}
    <div class="col-lg-4">
        <div class="card h-100">
            <div class="card-header">
    ...
    <div class="col-lg-4">
        <div class="card h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <a href="{{ route('admin.reviews') }}" style="font-size:12px;color:#3b82f6;text-decoration:none">View all →</a>
            </div>
                ...
                <div style="padding:10px 16px;border-bottom:1px solid #f1f5f9;display:flex;gap:10px;align-items:flex-start">
                    ...
                    <div style="flex:1;min-width:0">
                        <div style="display:flex;justify-content:space-between;align-items:center">
                            <span style="font-size:12px;font-weight:600;color:var(--text)">{{ Str::limit($rv->user_name, 14) }}</span>
                            <span style="color:#f59e0b;font-size:11px">{{ str_repeat('★', $rv->rating) }}{{ str_repeat('☆', 5 - $rv->rating) }}</span>
                        </div>
                        <div style="font-size:11px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                            {{ Str::limit($rv->product?->name ?? '—', 22) }}
                        </div>
                        @if($rv->comment)
                        <div style="font-size:11px;color:#64748b;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-style:italic">"{{ Str::limit($rv->comment, 36) }}"</div>
                        @endif
                    </div>
                </div>
                @endforeach
...
(function(){
    ...
    const reviewsByDay       = @json($reviewsByDay);
