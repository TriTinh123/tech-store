@extends('layouts.app')
@section('title', 'Notifications — TechStore')
...
<div class="nf-wrap">
    ...
    <div class="nf-list">
        ...
        @php $sev = $notif->severity ?? 'info'; $c = $sevColor[$sev] ?? '#0984e3'; $bg = $sevBg[$sev] ?? '#dbeafe'; @endphp
        <div class="nf-item {{ !$notif->read ? 'unread' : '' }}">
            <div class="nf-ico" style="background:{{ $bg }}">
                <i class="fas {{ $sevIcon[$sev] ?? 'fa-bell' }}" style="color:{{ $c }}"></i>
            </div>
            <div class="nf-body">
                <div class="nf-row">
                    <span class="nf-subject">{{ $notif->title }}</span>
                    @if(!$notif->read)<span class="nf-new-badge">NEW</span>@endif
                </div>
                <p class="nf-msg">{{ $notif->message }}</p>
                <div class="nf-meta">
                    <span><i class="far fa-clock"></i> {{ $notif->created_at->diffForHumans() }}</span>
                    @if($notif->action_url)
                    <a href="{{ $notif->action_url }}">{{ $notif->action_label ?? 'View' }} →</a>
                    @endif
                </div>
            </div>
            @if(!$notif->read)
    ...
    <div class="nf-pagination">{{ $notifications->links() }}</div>
    @endif
