...
<div class="container mt-5">
    ...
    <div class="text-center py-5">
        ...
        <a href="{{ route('products.index') }}" class="btn btn-primary mt-2">Explore products</a>
    </div>
    @else
    <div class="row g-3">
        @foreach($items as $item)
        @if($item->product)
        <div class="col-6 col-md-4 col-lg-3" id="wish-card-{{ $item->product_id }}">
            <div class="card h-100 shadow-sm" style="border-radius:10px;overflow:hidden;border:none">
                <div style="position:relative">
                    @if($item->product->image)
                    <img src="{{ asset('storage/'.$item->product->image) }}" class="card-img-top" style="height:180px;object-fit:cover" alt="{{ $item->product->name }}">
                    @else
                    ...
                    @endif
                    <button onclick="removeWish({{ $item->product_id }})"
                ...
                <div class="card-body p-2">
                    <div style="font-size:12px;font-weight:600;color:#374151;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="{{ $item->product->name }}">
                        {{ $item->product->name }}
                    </div>
                ...
                <div class="card-footer bg-white border-top-0 pt-0 pb-2 px-2 d-flex gap-1">
                    <a href="{{ route('product.show', $item->product_id) }}" class="btn btn-sm btn-outline-secondary flex-fill" style="font-size:11px">Details</a>
                    <form action="{{ route('cart.add', $item->product_id) }}" method="POST" class="flex-fill">
                        @csrf
