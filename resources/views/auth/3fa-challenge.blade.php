...
<head>
    ...
    <title>3FA Verification — {{ config('app.name') }}</title>
    <link href="https://fonts.bunny.net/css?family=poppins:300,400,500,600,700" rel="stylesheet"/>
    {{-- face-api.js: real face detection for biometric Factor 3 --}}
...
<div class="card">
    ...
    <div class="body">
        ...
        <div class="risk-box risk-{{ $level }}">
            <div class="risk-header">
                <div>
                    <span class="risk-badge badge-{{ $level }}">
                        {{ strtoupper($level) }} RISK
                    </span>
                </div>
                <div class="risk-score-wrap">
                    <span class="risk-score-num">{{ $numeric }}</span>
        ...
        {{-- ── Panel 2: Biometric Simulation ───────────────────────── --}}
        <div class="tab-panel" id="panel-biometric">
            @error('biometric')
                <div class="alert-danger" style="margin-bottom:14px">❌ {{ $message }}</div>
            @enderror
            ...
            @if(! $user->face_descriptor)
                <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:12px 14px;font-size:12px;color:#92400e;margin-bottom:14px;">
                    ...
                    <a href="{{ route('auth.face.enroll.form') }}" style="color:#b45309;font-weight:700;">enroll your face</a>
...
<script>
    ...
    window.addEventListener('load', () => {
        setTimeout(() => {
            document.getElementById('riskBar').style.width = '{{ $numeric }}%';
        }, 300);
    });
