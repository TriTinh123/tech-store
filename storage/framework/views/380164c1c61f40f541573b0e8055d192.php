...
<?php $__env->startSection('page_title', 'Select Payment Method'); ?>
...
<div class="row g-4">
    ...
    <div class="col-lg-7">
        <div class="bg-white rounded-4 border p-4" style="border-color:var(--border)!important">
            ...
            <p class="text-muted mb-4" style="font-size:13px">Order <strong>#<?php echo e($order->id); ?></strong> — Choose a payment method, you can change it anytime before confirming.</p>

            <form action="<?php echo e(route('checkout.payment.process', $order)); ?>" method="POST" id="payForm">
                <?php echo csrf_field(); ?>

                
                <label class="gw-card <?php echo e(old('payment_gateway','cod')=='cod'?'active':''); ?>" onclick="selectGW(this)">
                    <span class="gw-badge badge-fast">Fastest</span>
                    <input type="radio" name="payment_gateway" value="cod" <?php echo e(old('payment_gateway','cod')=='cod'?'checked':''); ?> required>
                ...
                <label class="gw-card <?php echo e(old('payment_gateway')=='bank_transfer'?'active':''); ?>" onclick="selectGW(this)">
                    <span class="gw-badge badge-safe">High Security</span>
                    <input type="radio" name="payment_gateway" value="bank_transfer" <?php echo e(old('payment_gateway')=='bank_transfer'?'checked':''); ?> required>
                ...
                <label class="gw-card <?php echo e(old('payment_gateway')=='momo'?'active':''); ?>" onclick="selectGW(this)">
                    <span class="gw-badge badge-pop">Popular</span>
                    <input type="radio" name="payment_gateway" value="momo" <?php echo e(old('payment_gateway')=='momo'?'checked':''); ?> required>
                ...
                <label class="gw-card <?php echo e(old('payment_gateway')=='zalopay'?'active':''); ?>" onclick="selectGW(this)">
                    <input type="radio" name="payment_gateway" value="zalopay" <?php echo e(old('payment_gateway')=='zalopay'?'checked':''); ?> required>
    ...
    <div class="col-lg-5">
        <div class="summary-card">
            <h6 class="fw-700 mb-3"><i class="fas fa-receipt me-2 text-success"></i>Order #<?php echo e($order->id); ?></h6>
            <div class="s-row"><span class="text-muted">Customer</span><span class="fw-600"><?php echo e($order->customer_name ?? ($order->user->name ?? 'N/A')); ?></span></div>
            <div class="s-row"><span class="text-muted">Deliver to</span><span style="max-width:55%;text-align:right;font-size:12px"><?php echo e($order->delivery_address ?? 'N/A'); ?></span></div>
            ...
            <div class="s-row">
                <span style="flex:1"><?php echo e(Str::limit($oi->product->name??'SP',28)); ?> <span class="text-muted">x<?php echo e($oi->quantity); ?></span></span>
                <span class="fw-600" style="color:var(--danger)">$<?php echo e(number_format($oi->subtotal, 2)); ?></span>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ADMIN\ecomerce\resources\views/payment-method.blade.php ENDPATH**/ ?>